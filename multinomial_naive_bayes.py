...
def main(root,mode):
	...
	author_words = {}
	for author_name in class_count.keys():
		author_words[author_name] = create_count_dict(author_name,mode)
	...
	if mode == 1:
		for filename in os.listdir('./tokenized_test'):
			...
			probability={}
			for author_name in class_count.keys():
				probability[author_name]=likelihood(filepath,author_name,author_words[author_name],class_count,total_class_count,word_count,mode)
	...
	for author in authors_list:
		...
		for i in range(len(authors_list)):
			fp = fp + contingency[i][index_a]
		fp = fp - tp
		print fp
		print str(predicted_class.count(author)-tp)
		fn = 0
		for i in range(len(authors_list)):
			fn = fn + contingency[index_a][i]
		fn = fn - tp
		print fn
		print str(sum(contingency[index_a]) - tp)
...
def likelihood(filename,author_name,author_words,class_count,total_class_count,word_count,mode):
	words = []
	...
	for w in words:
	    if w in author_words.keys():
	        freq = author_words[w] + 1
	    else:
	        freq = 1
		div = float(freq) / (author_words['count']+word_count)
...
def create_count_dict(author_name,mode):
	if mode == 1:
	...
	for line in author_file:
		...
		if l in train_words:
			count = train_words[l]
			train_words[l] = count + 1
		else:
			train_words[l] = 1

		count = count + 1
	author_file.close()
	train_words['count'] = count
	return train_words
